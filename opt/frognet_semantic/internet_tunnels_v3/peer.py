...
# [VOUCH_TRANSIT_GATE_V1] Cached broker transit map: relay /24-prefix
# ("10.160.160") -> list of /24 CIDRs that relay can transit (owned +
# transit_subnets), fetched each poll from GET /api/v4/transit-map. The
# discovery engine (RealBroker.transits) reads this to reject vouches through
# a relay that does not actually transit the destination. Absent file => gate
# inert.
NODE_TRANSIT_PATH = "/var/lib/frognet-tunnel/node_transit.json"
...
def _bringup_install_routes(channel_name: str, iface: str,
    ...
    Chicken-and-egg.
    """
    trace_enter('peer._bringup_install_routes', channel_name=repr(channel_name), iface=repr(iface), remote_subnets=repr(remote_subnets))
    ...
    for sn in remote_subnets:
        ...
        # Match planner._winning_via for the tunnel case: empty via
        # when peer.1 is inside dest, else via=peer.1.  [SMART_ONLINK_V1]
        # onlink decided by _onlink_required (whether dev has a
        # connected route covering via).
        if peer_prefix and _peer_one_inside_dest(peer_prefix, sn):
            via = ""
            onlink = False
            shape = "direct"
        elif peer_one:
            via = peer_one
            onlink = _onlink_required(iface, via)
            shape = "relay"
        else:
            # Channel name didn't parse - fall back to old per-subnet
            # peer.1 behavior so we still install *something*. Committer
            # will correct shape on first successful tunnel observation.
            via = sn.replace(".0/24", ".1")
            onlink = _onlink_required(iface, via)
            shape = "fallback"
        ...
        if ok:
            ...
            if via:
                config.log.info(
                    "BRINGUP_ROUTE_INSTALL %s via %s dev %s onlink "
                    "shape=%s",
                    sn, via, iface, shape)
            else:
                config.log.info(
...
def _peer_one_inside_dest(peer_prefix: str, dest_cidr: str) -> bool:
    ...
    trace_enter('peer._peer_one_inside_dest', peer_prefix=repr(peer_prefix), dest_cidr=repr(dest_cidr))
    try:
...
def _onlink_required(dev: str, via: str) -> bool:
    ...
    trace_enter('peer._onlink_required', dev=repr(dev), via=repr(via))
    if not via:
        return False
    try:
...
def _ip_to_int(ip: str) -> int:
    trace_enter('peer._ip_to_int', ip=repr(ip))
    parts = ip.split(".")
    if len(parts) != 4:
...
def _ensure_frognet0():
    trace_enter('peer._ensure_frognet0')
...
def _live_chorus_ips() -> Set[str]:
    trace_enter('peer._live_chorus_ips')
...
def _add_chorus_ip(chorus_ip: str, subnet: str):
    trace_enter('peer._add_chorus_ip', chorus_ip=repr(chorus_ip), subnet=repr(subnet))
...
def _remove_chorus_ip(chorus_ip: str):
    trace_enter('peer._remove_chorus_ip', chorus_ip=repr(chorus_ip))
...
def _sync_chorus_ips(chorus_assignments: List[dict]):
    trace_enter('peer._sync_chorus_ips', chorus_assignments=repr(chorus_assignments))
...
def _subnet_gateway(subnet: str) -> str:
    ...
    trace_enter('peer._subnet_gateway', subnet=repr(subnet))
    net = subnet.split("/")[0]
...
def _probe_ip_for_subnet(subnet: str) -> str:
    ...
    trace_enter('peer._probe_ip_for_subnet', subnet=repr(subnet))
    if "/" not in subnet:
        return ""
...
def broker_get(path: str, params: dict = None) -> dict:
    trace_enter('peer.broker_get', path=repr(path), params=repr(params))
...
def broker_post(path: str, body: dict) -> dict:
    trace_enter('peer.broker_post', path=repr(path), body=repr(body))
    url = f"{config.BROKER_URL}{path}"
...
def _read_transit_sentinel():
    """[TRANSIT_FROM_WINNERS_V1] Read the transit /24s discovery derived from
    the winners it installed (/etc/sentinels/transit_subnets.tsv, one /24 per
    line). Returns a sorted list, or None if the sentinel is absent/unreadable
    so the caller can fall back to the legacy derivation until the first merge
    has run."""
    path = "/etc/sentinels/transit_subnets.tsv"
    try:
        with open(path) as f:
...
def _sync_transit_subnets():
    ...
    trace_enter('peer._sync_transit_subnets')
    transit = _read_transit_sentinel()
    if transit is None:
        # No merge has written the sentinel yet - fall back to the legacy
        # derivation so a daemon polling before first discovery still registers.
        try:
...
def _sync_transit_map():
    """[VOUCH_TRANSIT_GATE_V1] Fetch the pond's relay->reachable-/24 map from the
    broker and cache it atomically to NODE_TRANSIT_PATH for the discovery engine.
    Best-effort: on any failure the stale (or absent) file is left in place, which
    leaves the vouch gate inert rather than wrongly suppressing routes."""
    trace_enter('peer._sync_transit_map')
    try:
        resp = broker_get("/api/v4/transit-map", {"pubkey": config.PUBKEY})
    except Exception as e:
        config.log.warning("SYNC_TRANSIT_MAP: broker unreachable: %s", e)
        return
    m = resp.get("map", {})
    if not isinstance(m, dict):
        config.log.warning("SYNC_TRANSIT_MAP: malformed map: %r", type(m))
        return
    import tempfile
    path = NODE_TRANSIT_PATH
    os.makedirs(os.path.dirname(path), exist_ok=True)
    fd, tmp = tempfile.mkstemp(prefix=".node_transit.", suffix=".json",
    ...
    try:
        with os.fdopen(fd, "w") as f:
            ...
            os.fsync(f.fileno())
        os.replace(tmp, path)
        config.log.info("SYNC_TRANSIT_MAP: cached %d relays", len(m))
    except Exception as e:
        try:
            os.unlink(tmp)
        except OSError:
            pass
        config.log.warning("SYNC_TRANSIT_MAP: write failed: %s", e)
...
def _load_local_state():
    trace_enter('peer._load_local_state')
...
def _save_tunnel_state(channel_name: str, info: dict):
    ...
    trace_enter('peer._save_tunnel_state', channel_name=repr(channel_name), info=repr(info))
    import tempfile
...
def _remove_tunnel_state(tunnel_name: str):
    trace_enter('peer._remove_tunnel_state', tunnel_name=repr(tunnel_name))
...
def _enumerate_kernel_wg_ifaces() -> Dict[str, str]:
    ...
    trace_enter('peer._enumerate_kernel_wg_ifaces')
    out: Dict[str, str] = {}
    try:
...
def _channel_already_lan_reachable(remote_subnets: List[str]) -> bool:
    ...
    """
    trace_enter('peer._channel_already_lan_reachable', remote_subnets=repr(remote_subnets))
...
def _bring_up_tunnel(channel: dict) -> bool:
    ...
    trace_enter('peer._bring_up_tunnel', channel=repr(channel))
...
def _tear_down_tunnel(channel_name: str):
    ...
    trace_enter('peer._tear_down_tunnel', channel_name=repr(channel_name))
...
def _tear_down_orphan_iface(iface: str, pubkey: str, reason: str):
    ...
    trace_enter('peer._tear_down_orphan_iface', iface=repr(iface), pubkey=repr(pubkey), reason=repr(reason))
...
def _verify_one_tunnel(channel_name: str, info: dict) -> bool:
    trace_enter('peer._verify_one_tunnel', channel_name=repr(channel_name), info=repr(info))
    iface = info.get("interface", "")
...
def _verify_tunnels_parallel(channel_names) -> List[str]:
    trace_enter('peer._verify_tunnels_parallel', channel_names=repr(channel_names))
    ...
    def _worker(ch_name: str):
        trace_enter('peer._worker', ch_name=repr(ch_name))
...
def _read_discovered_hosts() -> Set[str]:
    trace_enter('peer._read_discovered_hosts')
    try:
...
def _have_internet() -> bool:
    """Ping 8.8.8.8 - cheap, single packet, 2s timeout."""
    trace_enter('peer._have_internet')
    r = subprocess.run(
...
def _node_has_own_uplink() -> bool:
    ...
    """
    trace_enter('peer._node_has_own_uplink')
    try:
        r = subprocess.run(
            ["ip", "-4", "-o", "addr", "show"],
            capture_output=True, text=True, check=False, timeout=2)
...
def _measure_tunnel_rtt(probe_ip: str, iface: str) -> Optional[int]:
    ...
    trace_enter('peer._measure_tunnel_rtt', probe_ip=repr(probe_ip), iface=repr(iface))
...
def _write_channel_map() -> None:
    ...
    """
    trace_enter('peer._write_channel_map')
    ch_to_iface = {
...
def _write_committer_sidecars(ch_to_iface: Dict[str, str]) -> None:
    ...
    TRACEROUTES_PATH.
    """
    trace_enter('peer._write_committer_sidecars', ch_to_iface=repr(ch_to_iface))
...
def _persist_last_broker_state(broker_names: Set[str],
    ...
    """
    trace_enter('peer._persist_last_broker_state', broker_names=repr(broker_names), broker_channels=repr(broker_channels))
    payload = {
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                pass
            raise
        config.log.info("LAST_BROKER_STATE_WRITE: ok path=%s names=%d",
...
def _load_last_broker_state() -> None:
    ...
    trace_enter('peer._load_last_broker_state')
    try:
...
def _run_committer() -> bool:
    ...
    trace_enter('peer._run_committer')
    ...
    config.log.info("COMMITTER_INVOKE: cmd=%s", cmd)
    t0 = time.monotonic()
...
def _reconcile_local_state_with_kernel() -> None:
    ...
    trace_enter('peer._reconcile_local_state_with_kernel')
    try:
        kernel_ifaces = set(
        ...
        return
    config.log.info("STATE_RECONCILE: kernel_ifaces=%s python_state=%s",
...
def _reconcile_bringup_phase() -> bool:
    ...
    """
    trace_enter('peer._reconcile_bringup_phase')
    t0 = time.monotonic()
    config.log.info("BRINGUP_PHASE: status=enter")
    changed = False

    # LAN-only mode: no broker, no tunnels to bring up. Signal ready and exit.
    if getattr(config, "BROKER_DISABLED", False):
        config.log.info("BRINGUP_PHASE: status=broker_disabled - LAN-only mode, no tunnels")
        _signal_bringup_ready()
        return False

    # Phase 0: Internet check
    if not _have_internet():
        config.log.info("BRINGUP_PHASE: status=no_internet - signaling ready with no changes")
        _signal_bringup_ready()
        return False

    # Phase 0b: own-uplink check.  A LAN-child node (default route
    # forwards through a parent FrogNet peer; only 10.x addresses
    # locally) passes the internet check but cannot originate
    # WireGuard handshakes: its outbound UDP would source from a
    # 10.x address that no router on the public internet will
    # forward.  Every broker tunnel handshake will time out after
    # 30s.  Skip cleanly instead of grinding through that.
    if not _node_has_own_uplink():
        config.log.info(
            "BRINGUP_PHASE: status=lan_only_node - no non-10.x "
            "interface, this node reaches FrogNet peers through a "
            "parent node and cannot originate broker tunnels")
        _signal_bringup_ready()
        return False
    ...
    try:
        ...
        config.log.warning("BRINGUP_PHASE: broker_unreachable err=%s", e)
        _signal_bringup_ready()
        return False
    channels = resp.get("channels", [])
    ...
    config.log.info("BRINGUP_PHASE: orphan_scan kernel_ifaces=%s known_ifaces=%s",
                     sorted(kernel_ifaces.keys()), sorted(known_ifaces))
    orphan_count = 0
    for iface, pubkey in sorted(kernel_ifaces.items()):
        if iface in known_ifaces:
            config.log.info("BRINGUP_PHASE: orphan_check iface=%s decision=keep reason=in_known_ifaces", iface)
            continue
        ...
        if active_for_ch and iface != active_for_ch:
            ...
            config.log.info("BRINGUP_PHASE: orphan_check iface=%s decision=keep reason=matches_broker ch=%s active_for_ch=%s",
    ...
    for ch_name in sorted(_active_tunnels.keys()):
        ...
        iface = info.get("interface", "")
        if not iface:
            # No iface recorded - state is structurally broken; only
            # option is teardown.  Cannot refresh what isn't there.
            skipped_unhealthy.append((ch_name, "no_iface_recorded"))
            config.log.warning(
                "STALE_HANDSHAKE_PURGE: ch=%s reason=no_iface_recorded "
                "action=teardown (refresh impossible without iface)",
                ch_name)
            _tear_down_tunnel(ch_name)
            changed = True
            continue
        age = wg_handshake_age(iface)
        if age is None:
            # wg show on this iface returned no latest-handshake line
            # - either iface vanished from kernel between cycles or
            # never handshook.  Refresh can't help; teardown.
            skipped_unhealthy.append((ch_name, "iface_gone_or_never_handshook"))
            config.log.warning(
                "STALE_HANDSHAKE_PURGE: ch=%s iface=%s "
                "reason=iface_gone_or_never_handshook action=teardown "
                "(refresh requires a live iface with at least one handshake)",
                ch_name, iface)
            _tear_down_tunnel(ch_name)
            changed = True
            continue
        if age < config.HANDSHAKE_DEAD_SEC:
            # Tunnel handshake is fresh enough.  Leave it alone.
            continue

        # Stale handshake.  Try in-place refresh before destructive
        # teardown.  This preserves every route on the iface.
        from . import wg as wg_mod
        config.log.info(
            "STALE_HANDSHAKE_REFRESH: ch=%s iface=%s pre_age=%ds "
            "threshold=%ds wait=%ds - attempting in-place re-handshake "
            "(no route disturbance)",
            ch_name, iface, int(age), config.HANDSHAKE_DEAD_SEC,
            config.REFRESH_HANDSHAKE_WAIT_SEC)
        new_age = wg_mod.refresh_handshake(
            iface, config.REFRESH_HANDSHAKE_WAIT_SEC)
        if new_age is not None:
            refreshed_in_place.append((ch_name, iface, int(new_age)))
            config.log.info(
                "STALE_HANDSHAKE_REFRESH: ch=%s iface=%s OK pre_age=%ds "
                "new_age=%ds - routes preserved, no teardown",
                ch_name, iface, int(age), int(new_age))
            continue

        # Refresh failed - peer genuinely not responding.  Fall back
        # to destructive teardown.  This is the ONLY path in V2 that
        # removes routes en masse on a still-recorded iface.
        stale_purged.append((ch_name, iface, "refresh_failed_handshake_dead"))
        config.log.warning(
            "STALE_HANDSHAKE_PURGE: ch=%s iface=%s age=%ds threshold=%ds "
            "reason=refresh_failed_handshake_dead action=teardown "
            "(peer did not produce a fresh handshake within %ds of "
            "PersistentKeepalive kick; treating as actually-dead)",
            ch_name, iface, int(age), config.HANDSHAKE_DEAD_SEC,
            config.REFRESH_HANDSHAKE_WAIT_SEC)
        _tear_down_tunnel(ch_name)
        changed = True
    ...
    # and why.
    if refreshed_in_place or stale_purged or skipped_unhealthy:
        config.log.info(
            "PHASE3_TUNNEL_TEARDOWN_SUMMARY: refreshed=%d purged=%d "
            "skipped_unhealthy=%d  refreshed_list=%s  purged_list=%s  "
            "skipped_list=%s",
            len(refreshed_in_place), len(stale_purged),
            len(skipped_unhealthy),
    ...
    if to_bring_up:
        ...
        for ch_name in to_bring_up:
            def _worker(name: str):
                trace_enter('peer._worker', name=repr(name))
                try:
...
def _write_handshake_rtts() -> None:
    ...
    trace_enter('peer._write_handshake_rtts')
    import json
    import os
    import tempfile
    ...
    config.log.info("HANDSHAKE_RTTS_WRITE: channels=%d skipped=%d path=%s",
...
def _signal_bringup_ready() -> None:
    ...
    trace_enter('peer._signal_bringup_ready')
...
def _reconcile_commit_phase() -> bool:
    ...
    trace_enter('peer._reconcile_commit_phase')
    t0 = time.monotonic()
    ...
    _write_channel_map()
    config.log.info("COMMIT_PHASE: channel_map_write dur_ms=%d",
    ...
    config.log.info("COMMIT_PHASE: state_reconcile dur_ms=%d",
...
def _reconcile_tunnels_inner() -> bool:
    ...
    trace_enter('peer._reconcile_tunnels_inner')
...
def reconcile_tunnels() -> bool:
    ...
    serialize."""
    trace_enter('peer.reconcile_tunnels')
    import fcntl
    try:
...
def reconcile_bringup_only() -> bool:
    ...
    trace_enter('peer.reconcile_bringup_only')
    import fcntl
    try:
        lock_f = open(_RECONCILE_LOCK_PATH, "w")
    except OSError as e:
...
def reconcile_commit_only() -> bool:
    ...
    trace_enter('peer.reconcile_commit_only')
    import fcntl
    try:
        lock_f = open(_RECONCILE_LOCK_PATH, "w")
    except OSError as e:
...
def run_poll_loop():
    trace_enter('peer.run_poll_loop')
    _ensure_frognet0()
    _load_local_state()
    config.log.info("Starting poll loop (interval=%ds)",
                    config.POLL_INTERVAL_SEC)
    # [STARTUP_COMMIT_V1] Adopted tunnels have live kernel ifaces and
    # state files, but /24 routes may have been torn down externally
    # (or never installed if the previous daemon exited before its
    # committer ran).  Fire the existing commit phase once so the
    # committer reinstalls routes for every adopted tunnel using the
    # same channel_map.json + observations pipeline a normal merge
    # cycle uses.  Errors are logged inside reconcile_commit_only();
    # failure does not prevent the poll loop from starting.
    try:
        config.log.info("STARTUP_COMMIT: firing commit phase for %d "
                        "adopted tunnel(s)", len(_active_tunnels))
        reconcile_commit_only()
    except Exception as e:
        config.log.exception("STARTUP_COMMIT: unhandled: %s", e)
