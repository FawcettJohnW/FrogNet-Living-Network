...
def _with_retry(fn):
    """Execute fn(conn) with one retry on a TRANSIENT failure.

    [CREDENTIAL_FAILURE_IS_FATAL_V1] This used to retry on ANY exception and
    then print and re-raise. A wrong password is not something a retry fixes: it
    produced two Access-denied lines in the MySQL log per call, a warning on
    stderr, and a daemon that carried on without a cache forever. Transient
    failures -- a dropped socket, a restarting MySQL, a refused connect -- still
    get their retry, which is what the retry is for. A credential or schema
    error stops the process instead.
    """
    try:
        return fn(_get_conn())
    except Exception as e1:
        if _cred.is_fatal(e1):
            _cred.fatal(e1, _FATAL_TAG)
        _kill_conn()
        try:
            return fn(_get_conn())
