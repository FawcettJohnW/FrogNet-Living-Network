...
def check_installed(tree):
    """[MEASURE ONLY WHAT YOU CAN NAME_V1]

    install.sh records the sha256 of every file it put into the tree. This
    reads that record and compares it to what is on disk now.

    It exists because three separate campaigns were wasted on a fleet where
    one node was running different code from the others, and nothing said
    so until the numbers disagreed -- or worse, did not disagree visibly.
    A benchmark that cannot name the code it measured is not a measurement.

    Returns a list of problems; empty means the tree is what was installed.
    """
    vf = os.path.join(tree, ".frognet-ai-version")
    if not os.path.exists(vf):
        return ["%s has no .frognet-ai-version -- run install.sh on this "
                "node" % tree]
    bad = []
    lines = [l.strip() for l in open(vf) if l.strip()]
    build, entries = lines[0], lines[1:]
    for line in entries:
        ...
        try:
            ...
            continue
        if got != want:
            bad.append("%s differs from the installed build %s\n"
                       "        on disk  %s\n        installed %s"
                       % (path, build, got[:16], want[:16]))
    return bad
...
def run_rank(a):
    ...
    # publishing.
    if a.store and a.rank == 0:
        try:
            from core import frognet_tuples as _T
            stale = [r for r in _T._values_raw("", a.store)
                     if (r.get("SensorType") or "").startswith(
                         "nb1.%s." % a.runid)]
            if stale:
                svcs = sorted({r.get("SensorType") for r in stale})
                print("runid %r already has %d row(s) in the store from an "
                      "earlier attempt: %s\n"
                      "A reused runid means a rank can rendezvous against a "
                      "dead peer's port. Use a fresh --runid on every rank, "
                      "or purge:\n"
                      "  python3 store_admin.py purge --runid %s --store %s"
                      % (a.runid, len(stale), ", ".join(svcs)[:200],
                         a.runid, a.store), file=sys.stderr)
                return 2
        except Exception as e:
            print("could not check the store for a stale runid: %s: %s"
...
def main():
    ...
    ap.add_argument("--no-verify", action="store_true",
                    help="run even though the tree differs from the "
                         "installed build. Records it in the result.")
    ...
    if not os.path.isdir(a.pypath):
        a.pypath = ""
    # Every real run checks the tree against what install.sh recorded.
    if not a.selftest and a.role != "merge":
        problems = check_installed(a.tree)
        if problems:
            print("this node's tree does not match its installed build:",
                  file=sys.stderr)
            for pb in problems:
                print("  %s" % pb, file=sys.stderr)
            print("\nRe-run install.sh here, or pass --no-verify if you know "
                  "why it differs.", file=sys.stderr)
            if not a.no_verify:
                return 2
    if a.selftest:
    ...
    if a.rank >= 0:
        ...
        if os.path.isdir(a.out):
            a.out = os.path.join(a.out, "r%d.%s%s.json" % (
                a.rank, a.backend,
