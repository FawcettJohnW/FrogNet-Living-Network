...
# Generated by setup_lillypad_v4.bash on $(date)
domain=$NETWORK_NAME
local=/$NETWORK_NAME/
expand-hosts
...
# [DHCP_EXCLUDE_FUTURE_IFACES_V1] The loop below can only exclude interfaces
# that EXIST right now. Tunnels are created later, by the tunnel daemon, one per
# broker channel -- so a node that grows a wg iface after this runs serves DHCP
# across that tunnel, handing addresses out of THIS node's LAN pool to hosts on
# the far side of the mesh. The shipped opts_only.conf stops at wg4 for exactly
# this reason: it was written when the node had four tunnels.
#
# So pre-declare the tunnel namespace as well as what is currently up. wg0-wg31
# covers any plausible channel count; naming an interface that does not exist is
# harmless to dnsmasq. frognet0 is the on-link chorus device and never serves.
{
    for _IF in $(ip -o link show 2>/dev/null | awk -F': ' '{print $2}' | cut -d@ -f1); do
        [[ "$_IF" == "lo" || "$_IF" == "$SERVED_IF" ]] && continue
        echo "no-dhcp-interface=$_IF"
    done
    for _N in $(seq 0 31); do
        [[ "wg$_N" == "$SERVED_IF" ]] && continue
        echo "no-dhcp-interface=wg$_N"
    done
    [[ "frognet0" != "$SERVED_IF" ]] && echo "no-dhcp-interface=frognet0"
} | sort -u >> /etc/dnsmasq.d/opts_only.conf
