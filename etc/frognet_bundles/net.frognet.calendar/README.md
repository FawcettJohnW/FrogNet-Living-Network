...
## Run — standalone desktop app (no browser, no box)
Pure-stdlib Python + Tk; runs on Windows, macOS, and Linux with nothing to install
(Linux needs the distro's `python3-tk`).
```
python3 app/calendar_app.py                                # local, offline
python3 app/calendar_app.py --connect family-calendar.frognet   # join on the mesh
```
Same app, two backends: **local** embeds the codex (instant, offline) or **net** is a
thin UnREST client to a codex on a node (the fabric does the codec; the app reads/writes
the shared element only).

## Android app
Native Kotlin client in `android/` — open the folder in Android Studio, Gradle sync, Run;
point `CalendarClient(base=…)` at the well-known name. See `docs/ANDROID_STUDIO.md`.

## Quick visual preview in a browser (dev only)
```
python3 dev/serve_web_test.py        # http://127.0.0.1:8888/  (same-origin UI + codex)
```
The browser path is a preview only — the real clients are the desktop app and Android.

## Test
```
python3 tests/test_calendar_codex.py     # unit + real-codec integration
./run_baseline.sh                        # tests + verb smoke, self-cleaning
```
