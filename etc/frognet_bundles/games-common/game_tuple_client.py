...
def _load_codex(game, bundles_root):
    ...
    common = os.path.join(bundles_root, "games-common")
    ...
    fname = {"connectfour":"connectfour_codex","hearts":"hearts_codex",
             "liarsdice":"liarsdice_codex","reversi":"reversi_codex",
             "backgammon":"backgammon_gamecodex"}.get(game)
    cls = {"connectfour":"ConnectFourCodex","hearts":"HeartsCodex",
           "liarsdice":"LiarsDiceCodex","reversi":"ReversiCodex",
           "backgammon":"BackgammonGameCodex"}.get(game)
    if not fname:
        raise ValueError(f"unknown game {game}")
    # Load by explicit file path -- robust against sys.path state. The codex file must exist in
    # this game's codex dir; if it doesn't, say so clearly (not a bare ModuleNotFoundError).
    modpath = os.path.join(cand, fname + ".py")
    if not os.path.isfile(modpath):
        raise FileNotFoundError(
            f"{game}: codex file missing: {modpath}\n"
            f"(This install is missing {fname}.py -- reinstall the current client.)")
    spec = importlib.util.spec_from_file_location(fname, modpath)
    mod = importlib.util.module_from_spec(spec)
    sys.modules[fname] = mod          # so the codex's own 'from backgammon_codex import' works
