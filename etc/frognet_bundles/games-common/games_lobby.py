...
def _live_presence(g: dict) -> list:
    ...
    if not isinstance(pres, dict):
        return out   # caller surfaces the malformed table; presence just empty here
...
def list_tables(T, dbhost) -> list:
    ...
    for service, title in GAME_SERVICES.items():
        try:
            got = T.get(service, "state", dbhost=dbhost, fresh_s=TABLE_FRESH_S)
        except Exception as e:
            # DO NOT swallow: a failed read is the single most important thing to surface.
            import traceback
            errors.append(f"{service}: {type(e).__name__}: {e}")
            traceback.print_exc()
            continue
        for r in got:
            g = r.get("value") or {}
            if not isinstance(g, dict):
                errors.append(f"{service}: malformed value at {r.get('scope')}: {type(g).__name__}")
                continue
    ...
    list_tables.last_errors = errors   # caller (the hub) can read + display these
    return rows
